...
export class Dispatcher {
  ...
  private beforeMiddleware: MiddlewareFunc[] = []; // pre-processing 拦截/预处理
  private afterMiddleware: MiddlewareFunc[] = []; // post-processing 优化/修饰
  ...
  public async dispatch(event: Event, context: Context): Promise<Action[]> {
    try {
      ...
      let actions = await this.runMiddlewareChain(this.beforeMiddleware, event, context, () =>
  ...
  // Execute the handler for the given key
  ...
  // Get the handler key based on event and pipeline
